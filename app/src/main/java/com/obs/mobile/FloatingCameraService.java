...
public class FloatingCameraService extends Service {
    ...
    private View createProgrammaticLayout() {
        // Create a clean, rounded programmatic layout
        ...
        roundedBackground.setCornerRadius(dpToPx(12)); // Rounded corners
        roundedBackground.setColor(0xFF000000); // Black background
        roundedBackground.setStroke(dpToPx(2), 0xFF333333); // Dark border
        container.setBackground(roundedBackground);

        container.setPadding(dpToPx(6), dpToPx(6), dpToPx(6), dpToPx(6));

        // Create TextureView with rounded corners
        textureView = new TextureView(this);
        FrameLayout.LayoutParams textureParams = new FrameLayout.LayoutParams(
            FrameLayout.LayoutParams.MATCH_PARENT,
            FrameLayout.LayoutParams.MATCH_PARENT
        );
        textureParams.setMargins(0, dpToPx(30), 0, dpToPx(20)); // Leave space for controls
        container.addView(textureView, textureParams);

        // Create minimal top control bar
        createMinimalTopBar(container);

        // Create minimal bottom indicator
        createMinimalBottomBar(container);

        return container;
    }

    private void createMinimalTopBar(FrameLayout container) {
        // Minimal top control bar
        LinearLayout topBar = new LinearLayout(this);
        topBar.setOrientation(LinearLayout.HORIZONTAL);
        topBar.setBackgroundColor(0x88000000); // Semi-transparent
        topBar.setPadding(dpToPx(8), dpToPx(4), dpToPx(8), dpToPx(4));

        // Create rounded background for top bar
        android.graphics.drawable.GradientDrawable topBarBg = new android.graphics.drawable.GradientDrawable();
        topBarBg.setShape(android.graphics.drawable.GradientDrawable.RECTANGLE);
        topBarBg.setCornerRadius(dpToPx(8));
        topBarBg.setColor(0x99000000);
        ...
        topBarParams.setMargins(dpToPx(4), dpToPx(4), dpToPx(4), 0);
        container.addView(topBar, topBarParams);

        // Camera switch button (left)
        ImageButton btnSwitchCam = createRoundButton(android.R.drawable.ic_menu_rotate);
        LinearLayout.LayoutParams switchParams = new LinearLayout.LayoutParams(dpToPx(20), dpToPx(20));
        switchParams.setMargins(0, 0, dpToPx(6), 0);
        topBar.addView(btnSwitchCam, switchParams);

        btnSwitchCam.setOnClickListener(v -> {
            Log.d(TAG, "🔄 Switch camera clicked");
            isFrontCamera = !isFrontCamera;
            closeCamera();
            new Handler().postDelayed(this::openCamera, 300);
        });
        ...
        // Resize button
        btnResize = createRoundButton(android.R.drawable.ic_menu_crop);
        LinearLayout.LayoutParams resizeParams = new LinearLayout.LayoutParams(dpToPx(20), dpToPx(20));
        resizeParams.setMargins(dpToPx(6), 0, dpToPx(6), 0);
        topBar.addView(btnResize, resizeParams);

        // Close button (right)
        btnClose = createRoundButton(android.R.drawable.ic_menu_close_clear_cancel);
        LinearLayout.LayoutParams closeParams = new LinearLayout.LayoutParams(dpToPx(20), dpToPx(20));
        topBar.addView(btnClose, closeParams);
    }

    private void createMinimalBottomBar(FrameLayout container) {
        // Minimal bottom indicator
        LinearLayout bottomBar = new LinearLayout(this);
        bottomBar.setOrientation(LinearLayout.HORIZONTAL);
        bottomBar.setPadding(dpToPx(8), dpToPx(2), dpToPx(8), dpToPx(2));
        bottomBar.setGravity(Gravity.CENTER_VERTICAL);

        // Create rounded background for bottom bar
        android.graphics.drawable.GradientDrawable bottomBarBg = new android.graphics.drawable.GradientDrawable();
        bottomBarBg.setShape(android.graphics.drawable.GradientDrawable.RECTANGLE);
        bottomBarBg.setCornerRadius(dpToPx(6));
        bottomBarBg.setColor(0x99000000);
        bottomBar.setBackground(bottomBarBg);
        ...
        bottomParams.setMargins(0, 0, 0, dpToPx(4));
        container.addView(bottomBar, bottomParams);

        // Live indicator (small and clean)
        TextView liveIndicator = new TextView(this);
        liveIndicator.setText("● LIVE");
        liveIndicator.setTextColor(0xFFFF4444);
        liveIndicator.setTextSize(8);
        liveIndicator.setTypeface(null, android.graphics.Typeface.BOLD);
        bottomBar.addView(liveIndicator);
    }

    private ImageButton createRoundButton(int iconResource) {
        ...
        buttonBg.setColor(0x66FFFFFF);
        buttonBg.setStroke(1, 0x99FFFFFF);
